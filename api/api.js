...
app.post("/getCompanyID", async (req, res) => {
  ...
  res.send(
    await getSql.selectData("company", "company_id", `name = '${companyName}`)
  );
});
...
app.post("/insertInternship", async (req, res) => {
  ...
  const result = await getSql.selectData(
    "internship",
    "internship_id",
    `title = '${req.body.title}' and company_id = ${req.body.company_id} and student_id = ${req.body.student_id}`
  );
  res.send(result);
});
